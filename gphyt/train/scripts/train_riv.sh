...
#SBATCH --job-name=m-main-ar8-01

### Output file
#SBATCH --output=results/slrm_logs/m-main-ar8-01_%j.out
...
# debug=true
# Set up paths
base_dir="/scratch/zsa8rk/ML-Training-Suite"
python_exec="${base_dir}/gphyt/train/run_training.py"
log_dir="${base_dir}/results"
data_dir="/scratch/zsa8rk/datasets"
base_config_file="${base_dir}/gphyt/train/train.yml"
...
if [ "$new_training" = true ]; then
    ...
    echo "Config file found in $sim_dir, attempting restart..."
        ...
        cp $base_config_file $sim_dir/config.yaml

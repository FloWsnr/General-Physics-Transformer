...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def __init__(self, dim, base=10000):
        ...
        assert dim % 6 == 0, "Dimension must be divisible by 6 for time and spatial (x,y) embeddings"
        dim_per_component = dim // 3
        
        # Create separate frequency bands for time and spatial dimensions
        time_inv_freq = 1.0 / (base ** (torch.arange(0, dim_per_component, 2).float() / dim_per_component))
        spatial_inv_freq = 1.0 / (base ** (torch.arange(0, dim_per_component, 2).float() / dim_per_component))
    ...
    def forward(self, x):
        ...
        return (
            self.time_cos_cached, self.time_sin_cached,
            self.x_cos_cached, self.x_sin_cached,
            self.y_cos_cached, self.y_sin_cached
        )

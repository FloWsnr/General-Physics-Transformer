...
from gphyt.train.utils.train_vis import visualize_predictions
...
def test_visualize_predictions_handles_more_samples_than_available(
    ...
    files = list(save_path.parent.glob("*.png"))
    files += list(save_path.parent.glob("*.svg"))
    # num_samples is capped to batch_size (4), but randint can produce duplicate
    # indices which overwrite the same file, so count is non-deterministic.
    # We check: at least 1 sample (2 files) and at most 4 samples (8 files).
    assert 2 <= len(files) <= 4 * 2
